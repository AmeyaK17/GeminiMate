...
const QuizActionBar = () => {
    ...
    return (
        <ScrollArea className="h-[90vh] w-[50vw] rounded-md">
        <div className='min-h-full flex flex-col justify-evenly rounded-xl'>
            ...
            <ScrollArea className="h-[30vh] rounded-md">
                {topics.map((topic, index) => (
                    <div 
                        ...
                        className='p-2 cursor-pointer mb-2 rounded-full text-center gemini-gradient-topic hover:bg-gradient-to-r from-[#4b90ff] to-[#ff5546] hover:text-white transition-all duration-300'
                    >
                        {topic}
                    </div>
                ))}
            ...
            <h3 className='mt-8 text-center text-black text-lg'>Adjust Difficulty</h3>
            <Slider min={1} max={10} step={1} value={difficulty} setValue={setDifficulty} />

            <h3 className='mt-8 text-center text-black text-lg'>Adjust number of questions</h3>
